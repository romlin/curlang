...
check_curlang() {
  echo "Checking curlang installation..."
  which curlang || echo "curlang not found in PATH"
  if command -v pipx &>/dev/null; then
    pipx list | grep curlang || echo "curlang not installed via pipx"
  else
...
remove_dir "$SCRIPT_DIR/curlang.egg-info"
...
echo "Attempting to uninstall curlang from pipx..."
pipx uninstall curlang || true

echo "Setting up a virtual environment..."
python3 -m venv --prompt curlang_env "$SCRIPT_DIR/venv"
